...
class ProductFeedResourceIT {
    ...
    @Mock
    private ProductFeedRepository productFeedRepositoryMock;

    @Mock
    private ProductFeedService productFeedServiceMock;
    ...
    void getAllProductFeedsWithEagerRelationshipsIsEnabled() throws Exception {
        when(productFeedServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restProductFeedMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(productFeedServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllProductFeedsWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(productFeedServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
