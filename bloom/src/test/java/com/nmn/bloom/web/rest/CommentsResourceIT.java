...
class CommentsResourceIT {
    ...
    @Mock
    private CommentsService commentsServiceMock;
    ...
    void getAllCommentsWithEagerRelationshipsIsEnabled() throws Exception {
        when(commentsServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restCommentsMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(commentsServiceMock, times(1)).findAllWithEagerRelationships(any());
    }
